...
\begin{figure}
\lstinputlisting[style=castyle, firstline=11, lastline=31]{../src/bus/bus.h}
...
\begin{itemize}
  \item using a cache, which, beside its main goal, also performs many subsequent sequential accesses when transferring a block, and separes instruction and data flows;
...
Implementing the fast access mode thus is very strategic to obtain the best performances from \memoryname{} DRAM, even if this leaves space for further improvements with caches.
